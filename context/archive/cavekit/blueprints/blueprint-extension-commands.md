...
### R15: Convergence Command (/ck:convergence)

**Description:** A standalone command that parses the loop log and displays convergence analysis: pass rate trends, change velocity, and ceiling detection with actionable recommendations.

**Acceptance Criteria:**
- [ ] AC-1: Running `/ck:convergence` parses `context/impl/loop-log.md` and displays per-iteration pass rates.
- [ ] AC-2: The command classifies the current state as "progressing", "converging", or "ceiling".
- [ ] AC-3: When a ceiling is detected, actionable recommendations are displayed.
- [ ] AC-4: Running `/ck:convergence` without a loop log displays a usage message.

### R16: LLM-Callable Tools

**Description:** The extension registers tools callable by the LLM during subagent sessions: kit reading, build site status, acceptance checking, and convergence checking. These give subagents self-monitoring capabilities.

**Acceptance Criteria:**
- [ ] AC-1: `kit_read` tool returns parsed kit content for a given domain.
- [ ] AC-2: `build_site_status` tool returns current task statuses from the build site.
- [ ] AC-3: `acceptance_check` tool evaluates acceptance criteria for a given requirement.
- [ ] AC-4: `convergence_check` tool returns current convergence state from the loop log.

### R17: Failed Task Retry

**Description:** Failed tasks are retried up to a configurable maximum before being marked as blocked. The retry count is tracked per task and the `maxRetries` config setting is enforced.

**Acceptance Criteria:**
- [ ] AC-1: When a task fails and `retryCount < config.maxRetries`, the task status is reset to "pending" and `retryCount` is incremented.
- [ ] AC-2: When a task has exhausted retries (`retryCount >= config.maxRetries`), the task is marked "blocked".
- [ ] AC-3: The `maxRetries` config setting is read from the configuration system and applied by the wave executor.

### R18: Kit Parser / Draft Format Consistency

**Description:** The kit parser's requirement ID regex and the draft command's LLM prompt must agree on the requirement ID format to ensure end-to-end workflow integrity.

**Acceptance Criteria:**
- [ ] AC-1: The kit parser accepts requirement IDs in the format used by the draft command's LLM prompt template.
- [ ] AC-2: A kit generated by `/ck:draft` can be successfully parsed by the kit parser with zero parse errors for well-formed output.

### R19: Build Site Path Consistency

**Description:** All components that read build sites must use the same directory path that the architect command writes to, ensuring progress, hooks, and build commands operate on the same data.

**Acceptance Criteria:**
- [ ] AC-1: The progress command, compaction hook, and context injection hook read build sites from the same directory the architect command writes to.
- [ ] AC-2: A single canonical path constant or lookup function is used across all components.

### R20: Subagent Binary Name

**Description:** The wave executor must spawn subagent processes using the correct binary name for the current installation (cave or pi), not a hardcoded name.

**Acceptance Criteria:**
- [ ] AC-1: The wave executor spawns subagent processes using the binary name derived from the current process or configuration, not a hardcoded string.
- [ ] AC-2: On a Cave Pi installation where only `cave` is in PATH, subagent dispatch succeeds.

### R21: Safe Staging in Build Commits

**Description:** The build command must not use `git add -A` for wave commits. Only tracked files or explicitly scoped paths should be staged to prevent accidental commit of secrets.

**Acceptance Criteria:**
- [ ] AC-1: The build command does not stage untracked files outside of `context/` and source directories.
- [ ] AC-2: `.env` files, credentials, and other sensitive patterns are never staged by the build loop.

### R22: Stderr Handling in Subagent Dispatch

**Description:** Spawned subagent child processes must have their stderr consumed or inherited to prevent pipe buffer deadlocks.

**Acceptance Criteria:**
- [ ] AC-1: The wave executor either consumes stderr output or configures stdio to inherit stderr.
- [ ] AC-2: A subagent that writes >64KB to stderr does not hang the build.
...
| 2026-04-08 | Added R15-R22 — discovered during inspection (findings F-001 through F-006, F-002, F-003) |
